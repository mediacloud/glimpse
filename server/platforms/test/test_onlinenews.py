...
class OnlineNewsWaybackMachineProviderTest(unittest.TestCase):
    ...
    def test_domain_clause(self):
        ...
        results = self._provider.sample("coronavirus and canonical_domain:({})".format(domain),
                                        dt.datetime.strptime("2019-01-01", "%Y-%m-%d"),
                                        dt.datetime.strptime("2022-07-01", "%Y-%m-%d"))
        assert len(results) > 0
        for s in results:
            assert s['media_name'] == domain

    def test_big_domain_clause(self):
        ...
        results = self._provider.sample("canonical_domain:({})".format(" OR ".join(domains)),
